...
- [X] T182 Filter the frontend navigation menu so only modules/actions authorized by the connected user's permissions are rendered, instead of a static unfiltered `navItems` array, in `codebase/frontend/src/routes/Layout.tsx` per FR-019/US2-AC6 (missing)
