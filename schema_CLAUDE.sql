...
-- ---------------------------------------------------------------------
-- customer_returns : retour client (workflow 11 / règle 20.3),
-- décision explicite parmi 4 issues.
-- ---------------------------------------------------------------------
CREATE TABLE customer_returns (
    ...
    sale_order_line_id            UUID NOT NULL REFERENCES sale_order_lines(id) ON DELETE RESTRICT, -- RESTRICT : le retour doit toujours référencer la vente d'origine (règle 20.3.1)
...
CREATE INDEX ix_customer_returns_lot ON customer_returns (original_stock_lot_id);
CREATE TRIGGER trg_customer_returns_updated_at BEFORE UPDATE ON customer_returns
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
COMMENT ON TABLE customer_returns IS 'Retour client. Une décision "remise_stock" déclenche un stock_movement type=retour_client (référence polymorphe source_document_type=customer_return).';
-- =====================================================================
...
CREATE TRIGGER trg_notifications_updated_at BEFORE UPDATE ON notifications
    FOR EACH ROW EXECUTE FUNCTION set_updated_at();
COMMENT ON TABLE notifications IS 'Notifications persistées (poussées en direct via SignalR, retrouvables hors ligne — règle 5.19.3). Destinataire individuel ou par rôle.';
